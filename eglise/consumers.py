...
from .models import Discussion, Message, Appel
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    def get_old_messages(self, page=1, page_size=20):
        qs = Message.objects.filter(discussion__id=self.discussion_id).order_by("-date_envoye")
...
class CallConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        try:
            payload = {
                ...
                "avatar": (user.profile.image.url if getattr(user, "profile", None) and getattr(user.profile, "image", None) else None),
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        if msg_type in ["offer", "answer", "candidate", "ice"]:
            ...
            await self.channel_layer.group_send(
                self.room_group_name,
                {"type": "call_message", "message": {"type": "hangup", "user_id": self.scope["user"].id}, "sender": self.channel_name}
            )
